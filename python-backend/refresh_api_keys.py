...
def fetch_keys_once():
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False, args=["--window-position=-10000,0"])
        ...
        def handle_request(request):
            ...
            if "ac.cnstrc.com/autocomplete" in url:
        ...
        # Check that we got both keys, else raise
        if not keys["west_elm"]["key"] or not keys["pottery_barn"]["key"]:
            missing = []
            if not keys["west_elm"]["key"]:
