...
def fetch_west_elm_autocomplete(query="rug", num_suggestions=0, num_products=12, client_id=None, session="1"):
    """Call West Elm (Constructor.io) autocomplete API for a query.
    ...
    try:
        ...
        print(f"Error fetching West Elm autocomplete: {e}")
        return None
...
def extract_results(data, fallback_query="", include_suggestions=False):
    ...
    if include_suggestions:
        for item in sections.get("Search Suggestions", []) or []:
            ...
            search_url = f"https://www.westelm.com/search/results.html?words={quote_plus(title)}" if title else ""
            results.append({
                "title": title,
                "price": "",
                "url": search_url,
...
def save_to_csv(rows, filename="west_elm_results.csv"):
    if not rows:
...
def display_rows(rows, query):
    if not rows:
        ...
        return
    print(f"\n{'='*60}")
    print(f"WEST ELM RESULTS FOR '{query}' ({len(rows)} rows)")
    print(f"{'='*60}")
...
def main():
    query = "rug"
    print("Fetching West Elm autocomplete data...")

    # Ask API for only products (no suggestions) and request more than 4 products
    raw = fetch_west_elm_autocomplete(query=query, num_suggestions=0, num_products=12)
    if not raw:
    ...
    if rows:
        save_to_csv(rows, filename="west_elm_results.csv")
        display_rows(rows, query)
        print(f"\n{'='*60}")
        print("DATA EXPORT SUMMARY:")
        print(f"• CSV file: west_elm_results.csv ({len(rows)} rows)")
        print(f"{'='*60}")
    else:
