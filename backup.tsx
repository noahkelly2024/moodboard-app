...
const MoodBoardApp = () => {
  ...
  const dragRafRef = useRef<number | null>(null); // RAF throttle for dragging
  ...
  const addTextLayer = useCallback((slideIndex: number = currentSlide) => {
    ...
    setSlides(prev => prev.map((slide, idx) => idx === slideIndex ? { ...slide, layers: [...slide.layers, layer] } : slide));
  ...
  const moveLayerUp = useCallback((slideIndex: number, layerId: string | number) => {
    setSlides(prev => prev.map((slide, idx) => {
      ...
      const pos = layers.findIndex(l=>l.id===layerId);
      if (pos === -1 || pos === layers.length -1) return slide; // already top
  ...
  const moveLayerDown = useCallback((slideIndex: number, layerId: string | number) => {
    setSlides(prev => prev.map((slide, idx) => {
      ...
      layers.sort((a,b)=>a.zIndex - b.zIndex);
      const pos = layers.findIndex(l=>l.id===layerId);
      if (pos <= 0) return slide; // already bottom or not found
      [layers[pos], layers[pos-1]] = [layers[pos-1], layers[pos]];
      layers.forEach((l,i)=>{ l.zIndex = i; });
      return { ...slide, layers };
    }));
  ...
  const removeLayerFromSlide = useCallback((slideIndex: number, layerId: string | number) => {
    setSlides(prev => prev.map((slide, index) => {
      ...
      const filtered = slide.layers.filter(l => l.id !== layerId);
      // Normalize zIndex sequentially after deletion
      filtered
  ...
  const addSlide = useCallback(() => {
    const newSlide: Slide = {
      id: Date.now() + Math.random(),
      layers: [],
      backgroundColor: slides[slides.length - 1]?.backgroundColor || '#1f2937'
    };
  ...
  const handleLayerMouseDown = useCallback((e: React.MouseEvent, layerId: string | number) => {
    ...
    if (compositionCanvasRef.current) {
      ...
      const targetEl = e.currentTarget as HTMLElement; // layer element
      const layerRect = targetEl.getBoundingClientRect();
      setDragOffset({
        x: e.clientX - layerRect.left,
        y: e.clientY - layerRect.top
      });
    }
  }, [/* no deps aside from ref */]);
  ...
  const handleLayerMouseMove = useCallback((e: MouseEvent) => {
    if (!isDraggingLayer || !compositionCanvasRef.current) return;
    if (dragRafRef.current) cancelAnimationFrame(dragRafRef.current);
    dragRafRef.current = requestAnimationFrame(() => {
      const rect = compositionCanvasRef.current!.getBoundingClientRect();
      const layer = slides[currentSlide]?.layers.find(l => l.id === isDraggingLayer);
      if (!layer) return;
      const newLeftPx = e.clientX - rect.left - dragOffset.x;
      const newTopPx = e.clientY - rect.top - dragOffset.y;
      // Convert to percent using actual canvas pixel size
      const xPctRaw = (newLeftPx / rect.width) * 100;
      const yPctRaw = (newTopPx / rect.height) * 100;
      // Compute layer percent dimensions based on base design size 1200x675 used elsewhere
      const widthPct = (layer.size.width / 1200) * 100;
      const heightPct = (layer.size.height / 675) * 100;
      const x = Math.max(0, Math.min(100 - widthPct, xPctRaw));
      const y = Math.max(0, Math.min(100 - heightPct, yPctRaw));
      updateLayerProperty(currentSlide, isDraggingLayer, 'position', { x, y });
    });
  }, [isDraggingLayer, dragOffset, currentSlide, slides, updateLayerProperty]);
  ...
  const updateSlideBackgroundColor = useCallback((color: string) => {
    setSlides(prev => prev.map((slide, idx) => idx === currentSlide ? { ...slide, backgroundColor: color } : slide));
  }, [currentSlide]);
                      ...
                      <div className="card-dark rounded-lg shadow-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex space-x-1">
                            ...
                            <button
                              onClick={() => addTextLayer(currentSlide)}
                                ...
                                {slide.layers.map((layer) => {
                                  if (layer.type === 'text') {
                                    return (
                                      <div
                                        key={layer.id}
                                        className="absolute text-[6px] leading-tight px-1 py-0.5 rounded bg-black/30 text-white"
                                        style={{
                                          left: `${layer.position.x * 0.8}%`,
                                          top: `${layer.position.y * 0.8}%`,
                                          width: `${layer.size.width * 0.2}px`,
                                          height: `${layer.size.height * 0.2}px`,
                                          transform: `rotate(${layer.rotation}deg)`,
                                          opacity: layer.opacity,
                                          zIndex: layer.zIndex,
                                          textAlign: layer.textAlign,
                                        }}
                                      >{layer.text?.slice(0,15)}</div>
                                    );
                                  }
                                  const image = images.find(img => img.id === layer.imageId);
                                  if (!image) return null;
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center space-x-2">
                                    {layer.type === 'image' && (
                                      <img
                                        src={(images.find(img => img.id === layer.imageId)?.useProcessed && images.find(img => img.id === layer.imageId)?.processed) ? (images.find(img => img.id === layer.imageId)!.processed as string) : (images.find(img => img.id === layer.imageId)!.original)}
                                        alt="layer"
                                        className="w-8 h-8 object-cover rounded"
                                      />
                                    )}
                                    {layer.type === 'text' && (
                                      <div className="w-8 h-8 flex items-center justify-center bg-gray-700 rounded text-[10px] text-white truncate px-1">TXT</div>
                                    )}
                                    <div className="flex-1 min-w-0">
                                      <p className="text-white text-sm truncate">{layer.type === 'text' ? (layer.text || 'Text') : (images.find(img => img.id === layer.imageId)?.name || 'Image')}</p>
                                      <p className="text-xs text-gray-400">z {layer.zIndex}</p>
                                    </div>
                                    <div className="flex flex-col space-y-1">
                                      <button
                                        onClick={(e) => { e.stopPropagation(); moveLayerUp(currentSlide, layer.id); }}
                                        disabled={layer.zIndex === (Math.max(...currentSlideData.layers.map(l=>l.zIndex)))}
                                      ...
                                      <button
                                        onClick={(e) => { e.stopPropagation(); moveLayerDown(currentSlide, layer.id); }}
                    ...
                    <div className="flex-1">
                      <div className="card-dark rounded-lg shadow-lg p-4">
                        ...
                        >
                          {currentSlideData?.layers.map((layer) => {
                            if (layer.type === 'text') {
                              return (
                                <div
                                  ...
                                  className={`absolute flex items-center justify-center p-2 select-none cursor-move group ${selectedLayer===layer.id ? 'ring-2 ring-purple-400' : ''}`}
                                  style={{
                                    left: `${layer.position.x}%`,
                                    top: `${layer.position.y}%`,
                                    width: `${(layer.size.width / 1200) * 100}%`,
                                ...
                                >
                                  ...
                                  {/* Resize Handles */}
                                  {selectedLayer === layer.id && !isDraggingLayer && (
                                    <>
                                      <div onMouseDown={(e)=>handleResizeStart(e, layer.id, 'nw')} className="absolute -top-1 -left-1 w-3 h-3 bg-white rounded-sm shadow cursor-nw-resize" />
                                      <div onMouseDown={(e)=>handleResizeStart(e, layer.id, 'ne')} className="absolute -top-1 -right-1 w-3 h-3 bg-white rounded-sm shadow cursor-ne-resize" />
                                      <div onMouseDown={(e)=>handleResizeStart(e, layer.id, 'sw')} className="absolute -bottom-1 -left-1 w-3 h-3 bg-white rounded-sm shadow cursor-sw-resize" />
                                      <div onMouseDown={(e)=>handleResizeStart(e, layer.id, 'se')} className="absolute -bottom-1 -right-1 w-3 h-3 bg-white rounded-sm shadow cursor-se-resize" />
                                    </>
                                  )}
                                </div>
                              );
                            ...
                            return (
                              <div
                                key={layer.id}
                                onMouseDown={(e)=>handleLayerMouseDown(e, layer.id)}
                                className={`absolute select-none cursor-move group ${selectedLayer===layer.id ? 'ring-2 ring-purple-400' : ''}`}
                                style={{
                                  left: `${layer.position.x}%`,
                                  top: `${layer.position.y}%`,
                                  width: `${(layer.size.width / 1200) * 100}%`,
                          ...
                          {/* Background color picker hint */}
                          <div className="absolute bottom-2 left-2 flex items-center space-x-2 bg-black/30 rounded px-2 py-1 text-xs text-gray-200">
                            <span>BG:</span>
                            <input
                              type="color"
                              value={currentSlideData?.backgroundColor || '#1f2937'}
                              onChange={(e) => updateSlideBackgroundColor(e.target.value)}
                    ...
                    <div className="w-64">
                      <div className="card-dark rounded-lg shadow-lg p-4">
                        ...
                        {selectedLayer ? (() => {
                          ...
                          const image = layer.type === 'image' ? images.find(img => img.id === layer.imageId) : null;
                          return (
                            <div className="space-y-4">
                              ...
                              {layer.type === 'text' && (
                                <div>
                                  <p className="text-white text-sm font-medium mb-2">Text Layer</p>
                                  <textarea
                                    value={layer.text}
                                    onChange={(e) => updateLayerProperty(currentSlide, layer.id, 'text', e.target.value)}
                                    className="w-full h-24 text-sm bg-gray-700 border border-gray-600 rounded p-2 text-white resize-none"
                                  />
                                  <div className="mt-2 grid grid-cols-3 gap-2">
                                    <button onClick={() => updateLayerProperty(currentSlide, layer.id, 'textAlign', 'left')} className={`p-1 rounded ${layer.textAlign==='left'?'bg-purple-600':'bg-gray-700 hover:bg-gray-600'}`}> <AlignLeft className="w-4 h-4"/> </button>
                                    <button onClick={() => updateLayerProperty(currentSlide, layer.id, 'textAlign', 'center')} className={`p-1 rounded ${layer.textAlign==='center'?'bg-purple-600':'bg-gray-700 hover:bg-gray-600'}`}> <AlignCenter className="w-4 h-4"/> </button>
                                    <button onClick={() => updateLayerProperty(currentSlide, layer.id, 'textAlign', 'right')} className={`p-1 rounded ${layer.textAlign==='right'?'bg-purple-600':'bg-gray-700 hover:bg-gray-600'}`}> <AlignRight className="w-4 h-4"/> </button>
                                  </div>
                                </div>
                              )}
                              {/* Position Controls */}
                              ...
                              {layer.type === 'text' && (
                                <div className="space-y-2">
                                  ...
                                  <div>
                                    ...
                                    <input type="range" min="12" max="96" value={layer.fontSize || 32} onChange={(e)=> updateLayerProperty(currentSlide, layer.id, 'fontSize', parseInt(e.target.value))} className="w-full h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer" />
                                    <span className="text-xs text-gray-400">{layer.fontSize}px</span>
                                  </div>
                                  <div className="flex items-center space-x-2">
                                    <label className="text-xs text-gray-500">Color</label>
                                    <input type="color" value={layer.fontColor || '#ffffff'} onChange={(e)=> updateLayerProperty(currentSlide, layer.id, 'fontColor', e.target.value)} className="w-8 h-8 p-0 bg-transparent border-0" />
                                  </div>
                                  <div>
                                    <label className="text-xs text-gray-500">Font</label>
                                    <select value={layer.fontFamily} onChange={(e)=> updateLayerProperty(currentSlide, layer.id, 'fontFamily', e.target.value)} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-xs text-white">
                              ...
                              <div className="flex space-x-2 pt-2">
                                ...
                                <button
                                  onClick={() => { removeLayerFromSlide(currentSlide, layer.id); setSelectedLayer(null); }}
