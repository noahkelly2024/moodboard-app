...
const MoodBoardApp = () => {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'slideshow' | 'composition'>('grid');
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2 ml-4">
                  <button
                    ...
                    className={compositionMode ? 'btn-gradient text-sm px-3 py-1' : 'btn-gradient-outline text-sm px-3 py-1'}
                  >
                    <Layers className="w-4 h-4" />
                    <span className="ml-1 whitespace-nowrap">{compositionMode ? 'Composition' : 'Simple'}</span>
                  </button>
                  
                  <button
                    onClick={() => setViewMode(viewMode === 'grid' ? 'slideshow' : 'grid')}
                    className="btn-gradient-outline text-sm px-3 py-1"
                    disabled={compositionMode}
                  >
                    {viewMode === 'grid' ? <Play className="w-4 h-4" /> : <Grid className="w-4 h-4" />}
                    <span className="ml-1 whitespace-nowrap">{viewMode === 'grid' ? 'Slideshow' : 'Grid'}</span>
                  </button>
                </div>
